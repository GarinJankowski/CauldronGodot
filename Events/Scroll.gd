...
func init2(r):
	...
	scrollName = r.scrollName
	
	get_node("scroll/scroll").set_texture(load("res://UI/Gear/scrollIcon_" + scrollName + ".png"))
	get_node("scroll/Label").text = scrollName
	...
	get_node("scroll").rotation_degrees = rand_range(-2, 2)
	get_node("scroll").position.y += rand_range(-5, 5)
